...
# Agregado para las relaciones de tipos diferentes
class InterAgg(nn.Module):
	...
	def __init__(self, features, feature_dim, embed_dim, 
		...
		# Matriz de adyacencia de las tres relaciones 3*N_relaciones_i, i = 1:3
		self.adj_lists = adj_lists

		# Tres capa intra - relacionales para las tres tipos de relaciones en el grafo
		...
		self.inter = inter			# No se utiliza
		
		# Uso de GPU para las capas intra - relacionales
		self.cuda = cuda
	...
	#   Ejecuta un paso del agregador: se pasan los índices de los nodos, ya que se le carga el vector de características
	...
	def forward(self, nodes, labels, train_flag=True):
		...
		# Incluye en una lista todos los nodos que tienen una relación en común. La lista sería [[a], [b], [c]] donde
		# a, b y c son los nodos que comparte un tipo de relacion 
		to_neighs = []
		for adj_list in self.adj_lists:
			to_neighs.append([set(adj_list[int(node)]) for node in nodes])

		# Evita repetidos y organiza todos los nodos en una misma lista
		unique_nodes = set.union(set.union(*to_neighs[0]), set.union(*to_neighs[1]),
								 set.union(*to_neighs[2], set(nodes)))

		# Obtiene las características de los nodos y las clases positivas (fraudulentas)
		if self.cuda:
		...
		# Se calcula la similitud con una transformación lineal para la predicción. Se obtiene la puntuación del conjunto y de la clase fraudulenta
		batch_scores = self.label_clf(batch_features)
		pos_scores = self.label_clf(pos_features)

		# Indica los vecinos j de cada nodo i
		id_mapping = {node_id: index for node_id, index in zip(unique_nodes, range(len(unique_nodes)))}

		# Obtiene los scores de cada nodo para cada clase
		center_scores = batch_scores[itemgetter(*nodes)(id_mapping), :]
		
		
		# Convierte la lista de vecindad compuesta por sets en 3 diferentes compuestas por sub - listas
		#  El vector to_neighs es de la forma [[{}...{}], [{}...{}], [{}...{}]], donde {} es un set.
		#  En las siguientes instrucciones se pasa a tres listas del tipo [[]]
		r1_list = [list(to_neigh) for to_neigh in to_neighs[0]]
		r2_list = [list(to_neigh) for to_neigh in to_neighs[1]]
		r3_list = [list(to_neigh) for to_neigh in to_neighs[2]]
		
		
		# A cada vecino se le da la puntuación que sacó en cada relación
		...
		# Cuenta el número de vecinos para cada nodo
		...
		En este momento se tienen:
			- nodes: índices de los nodos seleccionados en el dataset original que se pasan a esta función
			- labels: etiquetas de cada nodo
			- r1_list: por cada nodo, todos sus vecinos
			- center_scores: score de cada nodo para las dos clases
			- rx_scores: scores de cada vecino j de un nodo i según la relación i. Se trata de un tensor de tensores donde
			cada sub - tensor indica la vecindad de un nodo con las puntuaciones del correspondiente vecino en las clases.
			- pos_scores: puntuaciones para las clases fraudulentas
			- rx_sample_num_list: número de vecinos para cada nodo i

		Estos datos se le pasan a los agregadores intra - clase para obtener su embedding.
		'''

		# Aplica la agregación intra - clase para cada relación
		# Eq. (8) in the paper
		...
		# Obtiene las características de los nodos
		...
		# Se aplica la función de activación RELU junto con una MLP (matriz de pesos) 
		combined = F.relu(cat_feats.mm(self.weight).t())

		return combined, center_scores


# Agregador para relaciones del mismo tipo
class IntraAgg(nn.Module):
	...
	def forward(self, nodes, batch_labels, to_neighs_list, batch_scores, neigh_scores, pos_scores, sample_list, train_flag):
		...
		# CHOOSE en entrenamiento: se conocen las etiquetas y la selección se hace en base a eso
		if train_flag:
		...
		# CHOOSE en test: no se conoce la etiqueta
		else:
			samp_neighs, samp_scores = choose_step_test(batch_scores, neigh_scores, to_neighs_list, sample_list)
		
		# Obtiene los nodos sin repeticiones y los almacena en un diccionario
		unique_nodes_list = list(set.union(*samp_neighs))
		unique_nodes = {n: i for i, n in enumerate(unique_nodes_list)}

		# Agregación de los nodos de una misma relación
		mask = Variable(torch.zeros(len(samp_neighs), len(unique_nodes)))
		column_indices = [unique_nodes[n] for samp_neigh in samp_neighs for n in samp_neigh]
		row_indices = [i for i in range(len(samp_neighs)) for _ in range(len(samp_neighs[i]))]
		
		# Matriz de adyacencia toda a 1s 
		mask[row_indices, column_indices] = 1
		
		if self.cuda:
			mask = mask.cuda()

		# Genera una máscara para la vecindad, de manera que dos nodos no vecinos no computarán para la agregación
		# de una tipo relación
		num_neigh = mask.sum(1, keepdim=True)
		mask = mask.div(num_neigh)  
		
		# Aplica la agregación del mensaje (con o sin cuda si no se tiene)
		if self.cuda:
		...
		# Genera las nuevas características
		agg_feats = mask.mm(embed_matrix)  # single relation aggregator

		# Las concatena a la de la capa anterior como en la Eq. 8
		cat_feats = torch.cat((self_feats, agg_feats), dim=1)  

		# Aplica la función de activación RELU según la Eq. 8
...
# CHOOSE: Selecciona los mejores vecinos para realizar la agregación de mensajes
def choose_step_neighs(center_scores, center_labels, neigh_scores, neighs_list, minor_scores, minor_list, sample_list, sample_rate):
	...
	# Bucle que recorre todos los nodos, seleccionando su índice (el número de nodo) y la puntuación de los vecinos
    for idx, center_score in enumerate(center_scores):
		# Puntuación del nodo que se está iterando
        center_score = center_scores[idx][0]

		# Puntuación de la vecindad
        neigh_score = neigh_scores[idx][:, 0].view(-1, 1)
        center_score_neigh = center_score.repeat(neigh_score.size()[0], 1)

		# Índices de la vecindad
        neighs_indices = neighs_list[idx]

		# Número de vecinos
        num_sample = sample_list[idx]

        # Calcula la distancia L1 para todos los nodos de la vecindad con el central (el que se está iterando)
        score_diff_neigh = torch.abs(center_score_neigh - neigh_score).squeeze()

		# Los ordena de menor a mayor y se pasa a lista
        ...
        # Se seleccionan los mejores según su distancia

		# Si se tienen mas vecinos en las puntuaciones que vecinos calculados, elige los mejores hasta el menor valor.
		# Si no, los coge todos
		...
		# Si el nodo es un nodo fraudulento, se sobre - muestrea, se eligen mas nodos fraudulentos para hacer la inferencia
		# aunque no estén conectados con el nodo que se está iterando
		...
		# Se obtiene los vecinos con su distancia
...
def choose_step_test(center_scores, neigh_scores, neighs_list, sample_list):
	...
	# Itera las puntuaciones en cada clase de los vecinos de un nodo
	for idx, center_score in enumerate(center_scores):
		center_score = center_scores[idx][0]
		neigh_score = neigh_scores[idx][:, 0].view(-1, 1)

		# Genera un vector con la clase predicha para calcular después la distancia Euclídea con los vecinos
		center_score = center_score.repeat(neigh_score.size()[0], 1)
		neighs_indices = neighs_list[idx]
		num_sample = sample_list[idx]

		# Calcula la distancia euclidea de los nodos con los vecinos a nivel de la clase y los ordena
		# Obtiene los que mas se parecen al nodo que se está analizando
