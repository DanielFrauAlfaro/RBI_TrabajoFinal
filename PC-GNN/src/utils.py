...
# Carga los datasets en función del nombre. Cargar el dataset de YELP o de AMAZON
...
# De todos los nodos y sus etiquetas selecciona cuales son los nodos fraudulentos
...
# PCIK: Calcula las probabilidades de que un nodo sea seleccionado. Se seleccionaran mayoritariamente nodos fraudulentos.
# La poca cantidad se compensa con un incremento en la probabilidad --> se divide por su ratio de aparición (lf_train)
def pick_step(idx_train, y_train, adj_list, size):
	# Adyacencia del nodo
    degree_train = [len(adj_list[node]) for node in idx_train]

	# Ratio de aparición de su clase
...
# Convierte la probabilidad en predicciones si la probabilidad supera cierto umbral (50%)
...
# Función para comprobar el conjunto de test o validación
def test_pcgnn(test_cases, labels, model, batch_size, thres=0.5):
	...
	for iteration in range(test_batch_num):
		...
		# Selecciona los nodos dentro del "batch"
		i_start = iteration * batch_size
		i_end = min((iteration + 1) * batch_size, len(test_cases))
		batch_nodes = test_cases[i_start:i_end]
		batch_label = labels[i_start:i_end]
		...
		# Pasa los valores a vectores de numpy
		gnn_prob_arr = gnn_prob.data.cpu().numpy()[:, 1]

		# Obtiene las predicciones según su probabilidad
		gnn_pred = prob2pred(gnn_prob_arr, thres)


		# Calcula las métricas acumuladas de cada "batch"
		...
		# Se insertan las predicciones y probabilidades a una lista
		gnn_pred_list.extend(gnn_pred.tolist())
		gnn_prob_list.extend(gnn_prob_arr.tolist())
